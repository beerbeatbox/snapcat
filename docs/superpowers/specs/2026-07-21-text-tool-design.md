...
- TextEditor โปร่งใสวางทับตำแหน่งจริง ฟอนต์/สีตรงกับผลลัพธ์, กรอบ accent บาง ๆ
- **handle ทั้งสามโชว์และลากได้ระหว่างพิมพ์** (feedback 2026-07-21): วงกลม
  ซ้าย/ขวา = wrap width, สี่เหลี่ยมขวาล่าง = ขนาดฟอนต์ — จับ handle ไม่นับเป็น
  คลิกออก, การเปลี่ยนระหว่างพิมพ์ ride snapshot ของ session (Esc ถอยทั้งชุด)
...
- **คลิกข้างนอกกรอบ** = unfocus/commit (ทางหลักในการจบ); ถ้า trim แล้ว
  ว่างเปล่า → ลบ annotation ทิ้ง + drop history snapshot (ไม่เหลือขยะใน undo)
- **คลิกที่จบ session ถูกกลืนทั้งคลิก** (feedback 2026-07-21): ไม่สร้างกล่องใหม่
  ไม่เลือก annotation อื่น ไม่วาดอะไร — คลิกครั้งถัดไปค่อยทำงานตามปกติ
- **สถานะ selected ก็เช่นกัน** (feedback 2026-07-22, เฉพาะ text tool): มีของถูก
  เลือกอยู่ + คลิก/ลากที่ว่าง = ปลด select เฉย ๆ (กลืน gesture) ไม่เปิดกล่องใหม่;
  คลิกบน annotation อื่นตรง ๆ ยังเลือกได้ปกติ — shape tools คงพฤติกรรมเดิม
  เพราะวาดเสร็จ auto-select ถ้ากลืนด้วยจะวาดรัวไม่ได้
